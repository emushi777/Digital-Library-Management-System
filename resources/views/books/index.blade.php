    ...
    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 20px;">
        ...
        @if($isAdmin)
            <div style="display:flex; gap:10px; align-items:center;">
                ...
                <form action="{{ url('/books/import') }}" method="POST" enctype="multipart/form-data" style="display:flex; gap:10px; align-items:center;">
                    @csrf
                    <input type="file" name="file" required>
                    <button type="submit" style="background:#F59E0B; color:white; padding:10px 15px; border:none; border-radius:6px; font-weight:600; cursor:pointer;">
                        Import CSV
                    </button>
                </form>

                @if($editMode)
                    <a href="{{ route('books.index') }}" style="background: #10B981; color: white; padding: 10px 20px; border-radius: 6px; text-decoration: none; font-weight: 600;">
                        Done Editing
                    </a>
            ...
            <tr style="border-bottom: 1px solid #F3F4F6;">
                <td style="padding: 12px; vertical-align: middle;">
                    @if($book->foto_kopertines && $book->foto_kopertines != 'default.jpg')
                        <img src="{{ asset('uploads/books/' . $book->foto_kopertines) }}" style="width: 50px; height: 70px; object-fit: cover; border-radius: 4px; border: 1px solid #E5E7EB;">
                    @else
                ...
                <td style="padding: 12px; vertical-align: middle;">
                    <div style="display: flex; align-items: center; justify-content: center; gap: 10px;">
                        <a href="{{ route('books.edit', $book->id) }}" style="background: #4F46E5; color: white; width: 80px; height: 30px; display:flex; align-items:center; justify-content:center; border-radius: 6px; text-decoration: none; font-size: 14px; font-weight: 600;">
                            Edit
                        </a>
                        ...
                        <form action="{{ route('books.destroy', $book->id) }}" method="POST" onsubmit="return confirm('Are you sure?')" style="margin: 0;">
                            @csrf
                            @method('DELETE')
                            <button type="submit" style="background: #EF4444; color: white; width: 80px; height: 30px; border: none; border-radius: 6px; cursor: pointer; font-size: 14px; font-weight: 600;">
                                Delete
                            </button>
                        </form>
                    </div>
