...
export default function Index({ auth, books, categories, authors, isAdmin, selectedCategory, selectedAuthor }) {
    ...
    const getRatingLabel = (book) => {
        if (!book.reviews_count) return 'No reviews yet';
        return `${Number(book.reviews_avg_vleresimi).toFixed(1)} / 5`;
    };

    const handleFilter = (type, id) => {
        const params = { category: selectedCategory, author: selectedAuthor };
        if (type === 'category') params.category = id;
        else if (type === 'author') params.author = id;
        Object.keys(params).forEach(key => params[key] === null && delete params[key]);
        router.get(route('books.index'), params, { preserveState: true, replace: true, only: ['books', 'selectedCategory', 'selectedAuthor'] });
    };
                    ...
                    <div className="mb-12">
                        <div className="flex justify-between items-center mb-6">
                            ...
                            <div className="flex gap-2">
                                <button onClick={() => scroll('left')} className="p-2 bg-white rounded-full shadow-sm hover:bg-gray-100 transition border border-gray-200">{'<'}</button>
                                <button onClick={() => scroll('right')} className="p-2 bg-white rounded-full shadow-sm hover:bg-gray-100 transition border border-gray-200">{'>'}</button>
                            </div>
                        </div>
                        ...
                        <div className="flex-1">
                            <div className="flex flex-col mb-8">
                                ...
                                <div className="flex flex-wrap gap-6 text-sm font-bold text-gray-400 uppercase tracking-tighter border-b border-gray-100 pb-4">
                                    <button onClick={() => handleFilter('category', null)} className={!selectedCategory ? 'text-blue-600 border-b-2 border-blue-600 pb-2' : 'pb-2 hover:text-gray-600'}>All Categories</button>
                                    {categories?.map((cat) => (
                                        <button key={cat.id} onClick={() => handleFilter('category', cat.id)} className={selectedCategory == cat.id ? 'text-blue-600 border-b-2 border-blue-600 pb-2' : 'pb-2 hover:text-gray-600'}>{cat.emertimi}</button>
                                    ))}
                                </div>
                            </div>
